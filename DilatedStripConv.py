...
"""Dilated Strip Convolution Module"""
...
class DiletedConv(nn.Module):
    def __init__(
        ...
        """
        A Dilated Strip Convolution Implementation
        ...
        # self.bn = nn.BatchNorm2d(2 * kernel_size)
        self.gn_offset = nn.GroupNorm(2 * kernel_size, 4 * kernel_size)
        self.gn = nn.GroupNorm(out_channels // 4, out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.tanh = nn.Tanh()

        self.offset_conv = nn.Conv2d(in_channels, 4 * kernel_size, 3, padding=1)  # 2* -> 4*
    ...
    def forward(self, input: torch.Tensor):
        ...
        # Run deformative conv
        y_coordinate_map, x_coordinate_map = get_coordinate_map_2D1(
            offset=offset,
            morph=self.morph,
            extend_scope=self.extend_scope,
            device='cuda',
...
def get_coordinate_map_2D1(
    ...
    batch_size, _, width, height = offset.shape
    kernel_size = offset.shape[1] // 2              # 9*2//2 -> 9*4//2
    center = kernel_size // 2 - 1                 # 9//2 -> 18//2
    device = torch.device(device)

    y_offset_, x_offset_ = torch.split(offset, kernel_size, dim=1)          #18 -> 2*9 ----> 36->2*18
    ...
    if morph == 0:
        ...
        for index in range(1, center // 2 + 1):
            y_offset_new_[center + index*2] = (
                y_offset_new_[center + (index - 1)*2] + y_offset_[center + index*2]
            )
            y_offset_new_[center - index*2] = (
                y_offset_new_[center - (index + 1)*2] + y_offset_[center - index*2]
            )
        ...
        for index in range(1, center // 2 + 1):
            x_offset_new_[center + index * 2] = (
                x_offset_new_[center + 2*(index - 1)] + x_offset_[center + 2*index]
            )
            x_offset_new_[center - index*2] = (
                x_offset_new_[center - 2*(index + 1)] + x_offset_[center - 2*index]
            )
